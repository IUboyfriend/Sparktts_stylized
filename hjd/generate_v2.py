...
SRC_ACTIVATIONS_SEMANTIC = None  # source activations, shape: [num_samples, num_layers, num_heads, head_dim]
TGT_ACTIVATIONS_SEMANTIC = None  # target activations, shape: [num_samples, num_layers, num_heads, head_dim]
SRC_ACTIVATIONS_GLOBAL = None  # source activations, shape: [num_samples, num_layers, num_heads, head_dim]
TGT_ACTIVATIONS_GLOBAL = None  # target activations, shape: [num_samples, num_layers, num_heads, head_dim]
...
SELECTED_HEADS_BY_LAYER_SEMANTIC = {}
SELECTED_HEADS_BY_LAYER_GLOBAL = {}
SELECTED_HEADS_BY_LAYER = {}
SS_RANK_GLOBAL = {}
SS_VH_GLOBAL = {}
SS_PROJ_SRC_MEAN_ACT_GLOBAL = {}
SS_PROJ_TGT_MEAN_ACT_GLOBAL = {}
SS_RANK_SEMANTIC = {}
SS_VH_SEMANTIC = {}
SS_PROJ_SRC_MEAN_ACT_SEMANTIC = {}
SS_PROJ_TGT_MEAN_ACT_SEMANTIC = {}
TOKENIZER = None


def     parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--activations_path_semantic", type=str, required=True)
    parser.add_argument("--activations_path_global", type=str, required=True)
    parser.add_argument("--test_folder",type=str,required=True)
    parser.add_argument("--selected_heads_path_semantic", type=str, required=True)
    parser.add_argument("--selected_heads_path_global", type=str, required=True)
    ...
    parser.add_argument("--output_folder", type=str, default="generated_audio_method_2", help="Folder to save generated audio files")
    # parser.add_argument("--save_dir", type=str, required=True)
    # # rank selection
    rank_group = parser.add_mutually_exclusive_group()
    rank_group.add_argument("--rank", type=int,default=12)
    rank_group.add_argument("--adaRank", action="store_true")
    parser.add_argument("--var_threshold", type=float, default=0.5)
    # acceleration
    parser.add_argument("--generation_method", type=str, choices=["baseline", "fast", "faster"], default="baseline")
    # # KNN
...
def svd_decomposition_global(rank=None, adaRank=False, var_threshold=None):
    ...
    global SS_RANK_GLOBAL, SS_VH_GLOBAL, SS_PROJ_SRC_MEAN_ACT_GLOBAL, SS_PROJ_TGT_MEAN_ACT_GLOBAL

    for layer_idx in SELECTED_HEADS_BY_LAYER_GLOBAL:
        for head_idx in SELECTED_HEADS_BY_LAYER_GLOBAL[layer_idx]:
            src_activations = SRC_ACTIVATIONS_GLOBAL[:, layer_idx, head_idx, :]
            tgt_activations = TGT_ACTIVATIONS_GLOBAL[:, layer_idx, head_idx, :]
            ...
            SS_RANK_GLOBAL[(layer_idx, head_idx)] = rank
            SS_VH_GLOBAL[(layer_idx, head_idx)] = Vh
            # print(SS_VH[(layer_idx, head_idx)].shape) # [k = 64, head dim = 64]
            SS_PROJ_SRC_MEAN_ACT_GLOBAL[(layer_idx, head_idx)] = torch.mean(proj_src_activations, dim=0)
            SS_PROJ_TGT_MEAN_ACT_GLOBAL[(layer_idx, head_idx)] = torch.mean(proj_tgt_activations, dim=0)
            # print(SS_PROJ_SRC_MEAN_ACT[(layer_idx, head_idx)].shape) # [64]

def svd_decomposition_semantic(rank=None, adaRank=False, var_threshold=None):
    # either rank or adaRank
    assert rank is not None or (adaRank is not None and var_threshold is not None)
    if adaRank:
        print("adaRank with var_threshold =", var_threshold)
    ...
    global SS_RANK_SEMANTIC, SS_VH_SEMANTIC, SS_PROJ_SRC_MEAN_ACT_SEMANTIC, SS_PROJ_TGT_MEAN_ACT_SEMANTIC   

    for layer_idx in SELECTED_HEADS_BY_LAYER_SEMANTIC:
        for head_idx in SELECTED_HEADS_BY_LAYER_SEMANTIC[layer_idx]:
            src_activations = SRC_ACTIVATIONS_SEMANTIC[:, layer_idx, head_idx, :]
            tgt_activations = TGT_ACTIVATIONS_SEMANTIC[:, layer_idx, head_idx, :]
            # print(src_activations.shape) # [792, 64]
            ...
            SS_RANK_SEMANTIC[(layer_idx, head_idx)] = rank
            SS_VH_SEMANTIC[(layer_idx, head_idx)] = Vh
            # print(SS_VH[(layer_idx, head_idx)].shape) # [k = 64, head dim = 64]
            SS_PROJ_SRC_MEAN_ACT_SEMANTIC[(layer_idx, head_idx)] = torch.mean(proj_src_activations, dim=0)
            SS_PROJ_TGT_MEAN_ACT_SEMANTIC[(layer_idx, head_idx)] = torch.mean(proj_tgt_activations, dim=0)
            # print(SS_PROJ_SRC_MEAN_ACT[(layer_idx, head_idx)].shape) # [64]

def get_steering_vector(layer_idx, head_idx, cur_activations, beta=0.80, global_flag=False):  # FIXME: beta is hard-coded
    # read from global variables
    if global_flag:
        rank = SS_RANK_SEMANTIC[(layer_idx, head_idx)]
        Vh = SS_VH_SEMANTIC[(layer_idx, head_idx)][:rank, :]
        proj_src_mean_act = SS_PROJ_SRC_MEAN_ACT_SEMANTIC[(layer_idx, head_idx)][:rank]
        proj_tgt_mean_act = SS_PROJ_TGT_MEAN_ACT_SEMANTIC[(layer_idx, head_idx)][:rank]
        beta = 1.25
    else:

        rank = SS_RANK_GLOBAL[(layer_idx, head_idx)]
        Vh = SS_VH_GLOBAL[(layer_idx, head_idx)][:rank, :]
        proj_src_mean_act = SS_PROJ_SRC_MEAN_ACT_GLOBAL[(layer_idx, head_idx)][:rank]
        proj_tgt_mean_act = SS_PROJ_TGT_MEAN_ACT_GLOBAL[(layer_idx, head_idx)][:rank]
        beta = 0.1
...
def edit_model_bias(model, cur_activations, global_flag):
    """
    model: model to edit
    ...
    if global_flag:
        SELECTED_HEADS_BY_LAYER = SELECTED_HEADS_BY_LAYER_SEMANTIC
    else:
        SELECTED_HEADS_BY_LAYER = SELECTED_HEADS_BY_LAYER_GLOBAL

    # print("SELECTED_HEADS_BY_LAYER", SELECTED_HEADS_BY_LAYER)
    for layer_idx, head_idx_list in SELECTED_HEADS_BY_LAYER.items():
        ...
        for head_idx in head_idx_list:
            ...
            steering_vector = get_steering_vector(layer_idx, head_idx, cur_head_activations, global_flag = global_flag)
...
def reset_model_global(model):
    """reset model bias to 0"""
    for layer_idx, heads in SELECTED_HEADS_BY_LAYER_GLOBAL.items():
...
def reset_model_semantic(model):
    """reset model bias to 0"""
    for layer_idx, heads in SELECTED_HEADS_BY_LAYER_SEMANTIC.items():
        zero_bias = torch.zeros(model.config.hidden_size, dtype=model.dtype, device=model.device)
        model.model.layers[layer_idx].self_attn.o_proj.bias = torch.nn.parameter.Parameter(zero_bias)

def generate(model, question_tokens, qa_prefix_tokens, max_length=400):
    ...
    for _ in range(max_length):
        with torch.no_grad():
            ...
            # print("tokens_without_template[0][-1]", TOKENIZER.decode(tokens_without_template[0][-1]))
            if tokens_without_template[0][-1] == 165156:
                print("--------------------------------global_flag--------------------------------")
                global_flag = True
            
            if global_flag:
                #edit global
                reset_model_semantic(model)
                cur_activations, _ = get_activations(model, tokens_without_template)
                edit_model_bias(model, cur_activations, global_flag)       
            else:
                #edit semantic
                reset_model_global(model)
                cur_activations, _ = get_activations(model, tokens_without_template)
                edit_model_bias(model, cur_activations, global_flag)
...
def main(args):
    ...
    global SRC_ACTIVATIONS_SEMANTIC, TGT_ACTIVATIONS_SEMANTIC, SRC_ACTIVATIONS_GLOBAL, TGT_ACTIVATIONS_GLOBAL
    print("Loading activations...")
    activations_global = np.load(args.activations_path_global)
    activations_global = torch.from_numpy(activations_global).to(model.device)
    source_indices = list(range(1, activations_global.shape[0], 2))
    target_indices = list(range(0, activations_global.shape[0], 2))
    source_activations = activations_global[source_indices]
    target_activations = activations_global[target_indices]
    source_activations = source_activations.to(model.device)
    SRC_ACTIVATIONS_GLOBAL = rearrange(source_activations, 'b l (h d) -> b l h d', h=model.config.num_attention_heads)
    target_activations = target_activations.to(model.device)
    TGT_ACTIVATIONS_GLOBAL = rearrange(target_activations, 'b l (h d) -> b l h d', h=model.config.num_attention_heads)
    # print("SRC_ACTIVATIONS_GLOBAL.shape", SRC_ACTIVATIONS_GLOBAL.shape)  # [792, 24, 14, 64]
    # print("TGT_ACTIVATIONS_GLOBAL.shape", TGT_ACTIVATIONS_GLOBAL.shape)  # [792, 24, 14, 64]



    activations_semantic = np.load(args.activations_path_semantic)
    activations_semantic = torch.from_numpy(activations_semantic).to(model.device)
    source_indices = list(range(1, activations_semantic.shape[0], 2))
    target_indices = list(range(0, activations_semantic.shape[0], 2))
    source_activations = activations_semantic[source_indices]
    target_activations = activations_semantic[target_indices]
    source_activations = source_activations.to(model.device)
    SRC_ACTIVATIONS_SEMANTIC = rearrange(source_activations, 'b l (h d) -> b l h d', h=model.config.num_attention_heads)
    target_activations = target_activations.to(model.device)
    TGT_ACTIVATIONS_SEMANTIC = rearrange(target_activations, 'b l (h d) -> b l h d', h=model.config.num_attention_heads)
    # print("SRC_ACTIVATIONS_SEMANTIC.shape", SRC_ACTIVATIONS_SEMANTIC.shape)  # [792, 24, 14, 64]
    # print("TGT_ACTIVATIONS_SEMANTIC.shape", TGT_ACTIVATIONS_SEMANTIC.shape)  # [792, 24, 14, 64]


    # load selected heads
    global SELECTED_HEADS_BY_LAYER_SEMANTIC, SELECTED_HEADS_BY_LAYER_GLOBAL, SELECTED_HEADS_BY_LAYER

    print("Loading selected heads...")
    selected_heads_semantic = np.load(args.selected_heads_path_semantic, allow_pickle=True)
    selected_heads_global = np.load(args.selected_heads_path_global, allow_pickle=True)
    # If it's stored as an object array, convert it to a list of tuples
    if isinstance(selected_heads_semantic, np.ndarray):
        selected_heads_semantic = [(int(layer), int(head)) for layer, head in selected_heads_semantic]
    if isinstance(selected_heads_global, np.ndarray):
        selected_heads_global = [(int(layer), int(head)) for layer, head in selected_heads_global]
    # group by layer for convenience and efficiency
    for layer_idx, head_idx in selected_heads_global:
        if layer_idx not in SELECTED_HEADS_BY_LAYER_GLOBAL:
            SELECTED_HEADS_BY_LAYER_GLOBAL[layer_idx] = []
        SELECTED_HEADS_BY_LAYER_GLOBAL[layer_idx].append(head_idx)
    print(SELECTED_HEADS_BY_LAYER_GLOBAL)
    for layer_idx, head_idx in selected_heads_semantic:
        if layer_idx not in SELECTED_HEADS_BY_LAYER_SEMANTIC:
            SELECTED_HEADS_BY_LAYER_SEMANTIC[layer_idx] = []
        SELECTED_HEADS_BY_LAYER_SEMANTIC[layer_idx].append(head_idx)
    print(SELECTED_HEADS_BY_LAYER_SEMANTIC)

    # determine the style subspace
    print("Determining the style subspace...")
    svd_decomposition_semantic(rank=args.rank, adaRank=args.adaRank, var_threshold=args.var_threshold)
    svd_decomposition_global(rank=args.rank, adaRank=args.adaRank, var_threshold=args.var_threshold)
    ...
    for index, sample_neutral in enumerate(prompts_neutral):
        ...
        tik = time.time()
        # print("sample_neutral.shape", sample_neutral.shape)
...
# python generate_v2.py --model_dir "../pretrained_models/Spark-TTS-0.5B" --activations_path_semantic "activations_v2/SparkTTS_head_wise_semantic.npy" --activations_path_global "activations_v2/SparkTTS_head_wise_global.npy" --test_folder "../dataset/last100_data" --selected_heads_path_semantic "edited_weights_semantic/top_heads_32_3.0.npy" --selected_heads_path_global "edited_weights_global/top_heads_20_3.0.npy" 

# python generate_v2.py --model_dir "../pretrained_models/Spark-TTS-0.5B" --activations_path_semantic "activations_v2/SparkTTS_head_wise_semantic.npy" --activations_path_global "activations_v2/SparkTTS_head_wise_global.npy" --test_folder "../dataset/last100_data" --selected_heads_path_semantic "edited_weights_semantic/top_heads_32_3.0.npy" --selected_heads_path_global "edited_weights_global/top_heads_128_3.0.npy" 